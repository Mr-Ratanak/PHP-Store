...
if(isset($_SESSION['user_id'])){
    ...
    include_once 'packages/database.php';
    $data = new Database();
    $category_slug = $_GET['category'];
    $category = $data->getSlugActive('categories',$category_slug);
    $cid = $category['id'];
    $check_category_exist = $data->countProByCategory($cid);
    
    
?>
    
       <h2 class="heading-quick mt-5">Category</div>
    <section class="wrap-category" >
        <?php 
        $fetch_product = $data->getProByCategory($cid);
        if($check_category_exist > 0){
            foreach($fetch_product as $row){
                ?>
                <form action="" method="post" id="add_tocart_form" class="main-category">
                ...
                <div class="sub-category">
                    ...
                    <h3 class="title"><?= substr($row['name'],0,27); ?></h3>
                    <div class="flex">
                        <span class="price">$<?= $row['price']; ?>/-</span>
                        <input class="qty" name="qty" id="qty" type="number" min="1" max="99" value="1">
                    </div>
                    <button type="submit" class="btn btn-primary addToCartBtn rounded-5 text-white" name="add_to_cart"><i class="fas fa-cart-plus"></i> Add To Cart</button>
            ...
            echo '<h3 class="text-center text-shadow">No category found!</h3>';
        }
        ?>
